...
# this python file is provided by https://github.com/lonicera-yx/MGT
...
def get_adj_metrix(adj, k):
    inf = 0xffff

    n_station = adj.shape[0]
    ...
    for n in range(n_station):
        for i in range(n_station):
            for j in range(n_station):
                if adj[i][n] + adj[n][j] <= adj[i][j]:
                    adj[i][j] = adj[i][n] + adj[n][j]

    return adj


def get_dist_metrix(adj_origin):
    sd = np.var(adj_origin)
    adj_origin = np.exp(-np.square(adj_origin) / sd)
    return adj_origin


def get_k_hop_metrix(adj, k):
    # without od flow
    adj = get_adj_metrix(adj, k)
    k_hop_metrix = get_dist_metrix(adj)
    return k_hop_metrix
